...
final class CollectionQuery
{
  ...
  public function search(string $query, array $fields = ['title', 'text']): static
  {
     ...
     *
     * Accepts a single value or an array of values (multi-select). Items
     * match when any of their comma-separated field values equals any of
     * the given values, which covers both plain and tags-like fields.
     */
  public function filter(string $field, mixed $value): static
  {
    ...
    $values = array_map(fn ($v) => trim((string) $v), is_array($value) ? $value : [$value]);

    $beforeCount = $this->collection->count();

    $this->collection = $this->collection->filter(function ($item) use ($field, $values) {
        $itemValues = array_map('trim', $item->{$field}()->split(','));
        return array_intersect($values, $itemValues) !== [];
    });
    $this->appliedFilters[$field] = $values;
  ...
  public function sort(string $field, string $direction = 'desc'): static
  {
     ...
     *
     * The requested page is clamped to the available range so that stale
     * URLs (e.g. page 3 of a search that now yields one page) degrade to
     * the closest valid page instead of an error page.
     */
  public function paginate(int $limit, string $param = 'p'): static
  {
    ...
    $limit = max(1, $limit);
    $requested = (int) (get($param) ?? 1);
    $totalPages = (int) ceil($this->totalCount / $limit);
    $page = min(max(1, $requested), max(1, $totalPages));
